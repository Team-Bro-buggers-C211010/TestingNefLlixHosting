...
const Profile = () => {
      ...
      <div>
        <div>
          ...
          <div className="absolute top-[20%] p-4 md:p-8">
            <h1 className="text-5xl md:text-7xl font-nans-bold my-2">My Shows</h1>
            <p className="font-nans-light text-gray-400 text-lg">{user.email}</p>
          </div>
        </div>
  ...
  <div key={movie.id} className="relative w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280px] inline-block rounded-lg overflow-hidden cursor-pointer m-2">
    <img
      ...
      src={createImageUrl(movie.backdrop_path || movie.poster_path, "w500")}  // Use || for fallback
      alt={movie.title}
    />
    <div className="absolute top-0 left-0 w-full h-40 bg-black/80 opacity-0 hover:opacity-100">
      ...
      <p>
        <AiOutlineClose 
          size={30} onClick={() => handleUnLikeShow(movie)} className="absolute top-2 right-2"
        />
      </p>
    </div>
  </div>
